...
表示すると、以下のようになります。  
![](./shots/html/html-2-1.png){width=200px}
...
前述の<a>タグや、<b>要素がインライン要素に相当します。

#### span

spanタグは、インライン要素のグルーピングに使われるタグで、
単体では大きな意味を持ちません。
他のインラインタグをまとめて、一つの要素として操作を行いたい場合に使用します。

## 画像を表示する

画像を表示するには、imgタグを使用します。

```html:image.html
<img src="画像のパス" alt="画像が表示されない場合に表示されるテキスト">
```

src要素には、画像に対するフルパスまたは相対パスを指定します。
...
フォームを構成するには、入力項目を form タグで囲むことで実現します。
...
formタグに下記の属性を指定することで、サーバにデータを送信できます。
...
実行すると以下のようになり、「ラベル」の部分を押下してもその横にあるラジオボタンを選択できるようになります。

![](./shots/html/html-2-3.png){width=80px}
...
順序のないリスト要素を表示するためには、ulタグとliタグを使用します。
